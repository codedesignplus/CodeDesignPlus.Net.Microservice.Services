...
5. Run the desired entry point:
   ...
   - For Worker:
      ```bash
      dotnet run --project src/entrypoints/CodeDesignPlus.Net.Microservice.Services.AsyncWorker
      ```
...
3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-services-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Services.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-services-worker ms-services-worker
   ```
