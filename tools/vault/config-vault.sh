...
vault kv put -mount=security-codedesignplus-keyvalue ms-services \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-services

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-services \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-services-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-services-mongo-role \
    db_name=db-ms-services \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-services" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-services-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-services-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-services-rabbitmq-role